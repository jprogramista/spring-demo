...
class SimpleMvcController(val oAuthService: OAuthService, val userService: UserService) {
    ...
    @GetMapping("/activate")
    fun activateUser(@RequestParam userId: Long, @RequestParam token: String, redirectAttributes: RedirectAttributes) : String {
        if (userService.activateUser(userId, token)) {
            redirectAttributes.addFlashAttribute("info", "user.activated")
        } else {
            redirectAttributes.addFlashAttribute("info", "user.notactivated")
        }
        return "redirect:/"
    }
}